...
class CartManager {
  ...
  async getCartById(id) {
    ...
    if (items.some(cart => cart.id === parseInt(id))) {
      return {code: 200, status: 'Ok', cart: items.find(cart => cart.id === parseInt(id))};
    }
    return {code: 400, status: 'Bad Request', message: 'Carrito no encontrado'};
  }
  ...
  async addProductToCart(cid, pid) {
    ...
    if (product.stock === 0) return {code: 400, status: 'Bad Request', message: 'El producto no cuenta con stock'};

    const itemsJSON = await fs.promises.readFile(this.path, 'utf-8');
    const items = JSON.parse(itemsJSON);
    //Verificamos si id corresponde a un carrito del arreglo
    if (items.some(cart => cart.id === parseInt(cid))) {
      const idx = items.findIndex(cart => cart.id === parseInt(cid));
      // Verificamos si existe algun producto igual
      if (items[idx].products.some(prod => prod.product === parseInt(pid))) {
        const idxProd = items[idx].products.findIndex(prod => prod.product === parseInt(pid));
        items[idx].products[idxProd].quantity = items[idx].products[idxProd].quantity + 1;
      } else {
      ...
      await fs.promises.writeFile(this.path, JSON.stringify(items));
      return {code: 200, status: 'Ok', message: 'Producto agreado al carrito'};
    }
    return {code: 400, status: 'Bad Request', message: 'Carrito no encontrado'};
  }
